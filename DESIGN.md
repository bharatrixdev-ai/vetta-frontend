...
## 19. Brand assets & link unfurling

**Favicon / tab icon** — `app/icon.svg`: the V on a dark rounded tile, served as SVG so it stays sharp at every size and in dark tab bars.
**Home-screen icon** — `app/apple-icon.tsx`: a 180×180 PNG generated by `next/og` for iOS "Add to Home Screen".
**Share card** — `app/opengraph-image.tsx` + `app/twitter-image.tsx`, both rendering the shared `OgCard` (1200×630): aurora wash, the V + wordmark, "Meet the people worth meeting.", the positioning line, and four proof chips ending on *Earned, not claimed.*
**Metadata** — `metadataBase`, a title template (`%s · Vetta`), description, keywords, full Open Graph block (type, siteName, locale `en_IN`), `summary_large_image` Twitter card, and `appleWebApp` settings.
